...
public class NeuralVisualizer : MonoBehaviour {
	...
	void Update () {
        ...
        if (selectedCreature)
        {
            selectionObject.transform.position = selectedCreature.transform.position;
            selectionObject.transform.localScale = 1.1f * selectedCreature.transform.localScale;
            NodeUpdate(selectedCreature.Model.inputs);
            infoText.text = selectedCreature.GetInfo();
    ...
    public List<double> NodeUpdate(List<double> inputs)
    {
        ...
        for (int i = 0; i < numHiddenLayers + 1; i++)
        {
            ...
            for (int j = 0; j < selectedCreature.Model.nNetwork.layers[i].numNeurons; j++)
            {
                double netInput = 0;

                int nNumInputs = selectedCreature.Model.nNetwork.layers[i].neurons[j].numInputs;
                ...
                for (int k = 0; k < nNumInputs; k++)
                {
                    ...
                    netInput += selectedCreature.Model.nNetwork.layers[i].neurons[j].weights[k] * inputs[weight];

                    //Set Lines
                    float line = (float)(selectedCreature.Model.nNetwork.layers[i].neurons[j].weights[k] * inputs[weight++]);
                ...
                netInput += selectedCreature.Model.nNetwork.layers[i].neurons[j].weights[nNumInputs] * -1;
